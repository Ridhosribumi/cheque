...
frappe.ui.form.on('Cheque In', {
	refresh: function(frm) {
		frm.fields_dict['bank_receive'].get_query = function(doc, dt, dn) {
			return{
				filters: [
		...
		if(frm.doc.docstatus == 1 && frm.doc.cheque_status == 'Rejected'){
			...
			function() {
				frappe.call({
					method: "cheque.cheque.doctype.cheque_in.cheque_in.delete_pe",
					args: {
					...
					callback: function(r){
						frm.reload_doc();
						frappe.call({
							method: "cheque.cheque.doctype.cheque_in.cheque_in.status_cancel",
							args: {
								name: frm.doc.name,
							},
							callback: function(r){
...
cur_frm.cscript['Journal Entry'] = function() {
	frappe.model.open_mapped_doc({
		method: "cheque.reference.make_journal_entry",
		frm: cur_frm
	})
}
	...
	var dialog = new frappe.ui.Dialog({
		...
		fields: [
			  {"fieldtype": "Data", "label": __("Cheque Status"), "fieldname": "cheque_status","reqd":1,"default":"Accepted","read_only":1},
				...
				{"fieldtype": "Link", "label": __("Bank Receive"), "fieldname": "bank_receive","options": "Account","reqd":1,"get_query": function(){return {filters: [["account_type",'=','Bank'],["is_group",'=',0]]};}},
